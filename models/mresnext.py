...
class ResNeXt(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, num_group=32, normalize=True):
        self.inplanes = 64
        ...
        self.linear = nn.Conv2d(512 * block.expansion, num_classes, kernel_size=1, bias=False)
        self.interp = nn.Upsample(size=(28, 28), mode='bilinear', align_corners=True)
...
def mresnext101(**kwargs):
    """Constructs a ResNeXt-101 model.
    """
    model = ResNeXt(Bottleneck, [3, 4, 26, 3], **kwargs)
    return model
...
if __name__ == "__main__":
    ...
    model = mresnext101(num_classes=2, normalize=True)
