...
def get_arguments():
    ...
    parser.add_argument('--attack_type', default='PGD', choices=["PGD", "FGSM", "MIM", "PGD400", "SPSA", "NES", "RayS"])
...
def test(epoch, model, testloader, criterion, attacker, ENV):
...
def evaluate_attack(model, testloader, attacker, ENV):
    ...
    if args.attack_choice == "Query" and args.attack_type == "RayS":
        torch_model = GeneralTorchModel(model, n_class=config.num_classes, im_mean=None, im_std=None)
    ...
    for batch_idx, batch in enumerate(testloader):
        ...
        if args.attack_choice == "BA" or args.attack_choice == "Query":
            ...
            if args.attack_type == "RayS":
                category_correct_adv += torch.sum(~succ)
            else:
                category_correct_adv += predicted.eq(cids).sum().item()
...
def main():
    ...
    criterion = nn.CrossEntropyLoss(ignore_index=config.ignare_label)
    ...
    if args.train:
        ...
        for epoch in range(starting_epoch, config.epochs):
            ...
            tc_loss, tc_acc = train(epoch, model, optimizer, trainloader, train_criterion, ENV)
            vc_acc, vc_acc_adv, vc_loss = test(epoch, model, testloader, criterion, attacker, ENV)
